let phoneRegex = /^\s*(?:\+?(\d{1,3}))?([-. (]*(\d{3})[-. )]*)?((\d{3})[-. ]*(\d{2,4})(?:[-.x ]*(\d+))?)\s*$/;  //TODO go to regexer.com or reg101.com and find a suitable phone number regex and place here
...
function checkRequired(id, message) {
  ...
  //if any of the elements is 'checked', return true.
    ...
    let valid = false;
    const elements = document.querySelectorAll('input[name="myCheckbox"]');
